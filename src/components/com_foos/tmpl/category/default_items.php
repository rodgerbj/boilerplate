...
<div class="com-foo-category__items">
	<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post" name="adminForm" id="adminForm">
		...
		<fieldset class="com-foo-category__filters filters btn-toolbar">
			<?php if ($this->params->get('filter_field')) : ?>
				<div class="com-foo-category__filter btn-group">
					<label class="filter-search-lbl sr-only" for="filter-search">
						<span class="badge badge-warning">
							<?php echo Text::_('JUNPUBLISHED'); ?>
						</span>
						<?php echo Text::_('COM_FOOS_FILTER_LABEL') . '&#160;'; ?>
					</label>
					<input
						type="text"
						name="filter-search"
						...
						title="<?php echo Text::_('COM_FOOS_FILTER_SEARCH_DESC'); ?>"
						placeholder="<?php echo Text::_('COM_FOOS_FILTER_SEARCH_DESC'); ?>"
					>
				</div>
			<?php endif; ?>

			<?php if ($this->params->get('show_pagination_limit')) : ?>
				<div class="com-foo-category__pagination btn-group float-right">
		...
		<?php if (empty($this->items)) : ?>
			<p>
				<?php echo Text::_('COM_FOOS_NO_FOOS'); ?>
			</p>
		<?php else : ?>
			...
			<ul class="com-foo-category__list category row-striped">
				<?php foreach ($this->items as $i => $item) : ?>
					...
					<?php if (in_array($item->access, $this->user->getAuthorisedViewLevels())) : ?>
							...
							<div class="col-md-2">
								<?php if ($this->items[$i]->image) : ?>
									<a href="<?php echo Route::_(RouteHelper::getFooRoute($item->slug, $item->catid, $item->language)); ?>">
										<?php echo HTMLHelper::_('image', $this->items[$i]->image, Text::_('COM_FOOS_IMAGE_DETAILS'), array('class' => 'foo-thumbnail img-thumbnail')); ?></a>
								<?php endif; ?>
							</div>
						<?php else : ?>
							<?php $foo_width = 9; ?>
						...
						<div class="list-title col-md-<?php echo $foo_width; ?>">
							<a href="<?php echo Route::_(RouteHelper::getFooRoute($item->slug, $item->catid, $item->language)); ?>">
						...
						<div class="col-md-3">
							<?php if ($this->params->get('show_telephone_headings') && !empty($item->telephone)) : ?>
								<?php echo Text::sprintf('COM_FOOS_TELEPHONE_NUMBER', $item->telephone); ?><br>
							<?php endif; ?>

							<?php if ($this->params->get('show_mobile_headings') && !empty ($item->mobile)) : ?>
									<?php echo Text::sprintf('COM_FOOS_MOBILE_NUMBER', $item->mobile); ?><br>
							<?php endif; ?>

							<?php if ($this->params->get('show_fax_headings') && !empty($item->fax) ) : ?>
								<?php echo Text::sprintf('COM_FOOS_FAX_NUMBER', $item->fax); ?><br>
							<?php endif; ?>
						</div>
						<?php if ($canEdit || ($canDo->get('core.edit.own') && $item->created_by == $userId)) : ?>
							<div><?php echo HTMLHelper::_('fooicon.edit', $item, $this->params); ?></div>
						<?php endif; ?>
			...
			<div class="com-foo-category__counter w-100">
